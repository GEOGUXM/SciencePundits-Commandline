...
// @callback([users])
function getStakeholders(record, callback) {
    if('stakeholders' in record) {
        callback(record.stakeholders);
    } else {
        $('#progress_msg').html('Finding stakeholders...');
        ...
        var details = {
            ...
            contentType: 'json',
            headers: { Accept: 'application/json' },
            url: info.base_url+'stakeholders/stakeholder/'+record.id+'/'
        };
        xhr_send(details, function(error, response) {
           if(error) {
                $('#error').html('Error getting stakeholders ('+error+')');
                $('#progress').hide();
            } else {
                callback(response.stakeholders);
            }
        });
    }
}
    ...
    getContent(url, function(data) {
        getKeywords(data, function(keyword_list) {
            getEntities(data, function(entity_list) {
                getStakeholders(data, function(stakeholder_list) {
                    ...
                    var details = {
                        ...
                        contentType: 'json',
                        headers: { Accept: 'application/json' },
                        url: info.base_url+'stakeholdertweets/stakeholder/'+data.id+'/'
                    }
                    xhr_send(details, function(error, response) {
                        if(error) {
                            $('#error').html(error);
